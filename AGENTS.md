...
- `SANEPI_AGENT_TYPE=explore pi -e ./src/index.ts` — manual local smoke test.
...
- Keep `SANEPI_AGENT_TYPE` compatibility because senpi background tasks set that env var.
- Keep `PI_AGENT_TYPE` compatibility for upstream-style extension stacks.
